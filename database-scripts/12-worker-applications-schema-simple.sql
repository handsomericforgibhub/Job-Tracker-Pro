-- Simplified Worker Applications Schema (PostgreSQL/Supabase Compatible)
...
CREATE INDEX idx_worker_applications_company ON worker_applications(company_id);
...
CREATE INDEX idx_worker_applications_applied_at ON worker_applications(applied_at);

-- Add sharing columns to job_assignments table
...
CREATE TABLE assignment_access_logs (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_assignment_access_logs_assignment ON assignment_access_logs(assignment_id);
CREATE INDEX idx_assignment_access_logs_token ON assignment_access_logs(share_token);
CREATE INDEX idx_assignment_access_logs_accessed_at ON assignment_access_logs(accessed_at);
...
CREATE INDEX idx_job_assignments_share_token ON job_assignments(share_token) WHERE share_token IS NOT NULL;
CREATE INDEX idx_job_assignments_public ON job_assignments(is_public) WHERE is_public = TRUE;
CREATE INDEX idx_job_assignments_expires ON job_assignments(share_expires_at) WHERE share_expires_at IS NOT NULL;
