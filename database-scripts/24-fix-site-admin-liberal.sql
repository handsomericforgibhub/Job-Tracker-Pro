-- Fix the update_job_status_safely function with liberal site admin detection
-- This version assumes any user not in the users table but in auth.users is a site admin
...
BEGIN
    ...
    IF user_company_id IS NULL THEN
        ...
        -- ASSUMPTION: If user exists in auth.users but not in users table, they're a site admin
        -- This is a common pattern for super-admin users
        is_site_admin := TRUE;
        user_role := 'site_admin';
        
        -- Alternative approaches to check for site admin:
        -- 1. Check raw_user_meta_data for role
        -- 2. Check raw_app_meta_data for role  
        -- 3. Check if email matches certain patterns
        -- 4. Check a separate admin table
        
        -- Try multiple methods to confirm site admin status
        SELECT CASE 
            WHEN raw_user_meta_data->>'role' = 'site_admin' THEN TRUE
            WHEN raw_app_meta_data->>'role' = 'site_admin' THEN TRUE
            WHEN user_metadata->>'role' = 'site_admin' THEN TRUE
            WHEN app_metadata->>'role' = 'site_admin' THEN TRUE
            WHEN email LIKE '%admin%' THEN TRUE
            ELSE TRUE  -- Default to TRUE for users not in users table
...
-- Update the check_user_status function to be more detailed
...
BEGIN
    ...
    -- Check auth.users table with all metadata
    SELECT email, raw_user_meta_data, raw_app_meta_data, user_metadata, app_metadata 
    INTO auth_email, auth_meta, auth_app_meta, user_meta, app_meta
    FROM auth.users 
    WHERE id = p_user_id;
    ...
    IF auth_email IS NOT NULL THEN
        ...
        result := result || 'User metadata: ' || COALESCE(user_meta::TEXT, 'NULL') || E'\n';
        result := result || 'App metadata: ' || COALESCE(app_meta::TEXT, 'NULL') || E'\n';
    ELSE
