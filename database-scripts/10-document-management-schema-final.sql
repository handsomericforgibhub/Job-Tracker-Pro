-- Document Management System Database Schema (FINAL SAFE VERSION)
...
-- Step 2: Create document categories table for organization
...
-- Step 3: Create main documents table (without task_id column initially)
CREATE TABLE IF NOT EXISTS documents (
    ...
    -- Note: task_id will be added in the next step
...
-- Step 4: Add task_id column to documents table if it doesn't exist
DO $$
BEGIN
...
-- Step 5: Add foreign key constraint for task_id
DO $$
BEGIN
...
-- Step 6: Create document access log for tracking views/downloads
...
-- Step 7: Create document comments for collaboration
...
-- Step 8: Insert default document categories for each company
DO $$
DECLARE
...
-- Step 9: Add updated_at triggers (only if they don't already exist)
DO $$
BEGIN
    IF NOT EXISTS (
    ...
    ELSE
        RAISE NOTICE 'ℹ️ Trigger for document_categories already exists';
    END IF;
    ...
    ELSE
        RAISE NOTICE 'ℹ️ Trigger for documents already exists';
    END IF;
    ...
    ELSE
        RAISE NOTICE 'ℹ️ Trigger for document_comments already exists';
    END IF;
END $$;
...
-- Step 12: Create trigger for share token management (only if it doesn't exist)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger 
    ...
    ELSE
        RAISE NOTICE 'ℹ️ Share token trigger already exists';
    END IF;
END $$;

-- Step 13: Final verification and setup complete message
DO $$
DECLARE
    task_columns_exist BOOLEAN;
    documents_table_exists BOOLEAN;
    documents_task_id_exists BOOLEAN;
...
BEGIN
    ...
    -- Check if documents table exists and has task_id column
    ...
    IF task_columns_exist THEN
        RAISE NOTICE '✅ Tasks table updated with required columns (parent_task_id, sort_order)';
    ELSE
    ...
    IF documents_task_id_exists THEN
        RAISE NOTICE '✅ Documents table has task_id column';
    ELSE
        RAISE NOTICE '⚠️ Documents table missing task_id column';
    END IF;

    IF fk_constraint_exists THEN
        RAISE NOTICE '✅ Foreign key constraint for task_id created';
    ELSE
        RAISE NOTICE '⚠️ Foreign key constraint for task_id not created';
    END IF;
    ...
    IF default_categories_count > 0 THEN
        RAISE NOTICE '✅ Default document categories inserted (% total categories)', default_categories_count;
    ELSE
        RAISE NOTICE '⚠️ No default categories found';
    END IF;

    RAISE NOTICE '';
    RAISE NOTICE '🎉 Document Management System database schema setup complete!';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '   2. Create document management pages';
    RAISE NOTICE '   3. Integrate with jobs and tasks';
END $$;