-- Multi-Worker Task Assignment System (Fixed)
...
-- Copy existing assignments to new table (only if assigned_to references a valid worker)
-- This handles the case where assigned_to might reference users instead of workers
INSERT INTO task_assignments (task_id, worker_id, assigned_by)
SELECT t.id, t.assigned_to, t.created_by 
FROM tasks t
WHERE t.assigned_to IS NOT NULL 
  AND EXISTS (
    SELECT 1 FROM workers w WHERE w.id = t.assigned_to
  )
ON CONFLICT (task_id, worker_id) DO NOTHING;

-- For tasks assigned to users (not workers), try to find the corresponding worker
-- and create the assignment if one exists
INSERT INTO task_assignments (task_id, worker_id, assigned_by)
SELECT t.id, w.id, t.created_by 
FROM tasks t
JOIN workers w ON w.user_id = t.assigned_to
WHERE t.assigned_to IS NOT NULL 
  AND NOT EXISTS (
    SELECT 1 FROM workers w2 WHERE w2.id = t.assigned_to
  )
  AND NOT EXISTS (
    SELECT 1 FROM task_assignments ta WHERE ta.task_id = t.id AND ta.worker_id = w.id
  )
ON CONFLICT (task_id, worker_id) DO NOTHING;
...
-- Display summary of migrated assignments
DO $$
DECLARE
    assignment_count INTEGER;
    migrated_worker_assignments INTEGER;
    migrated_user_assignments INTEGER;
BEGIN
    -- Count total assignments created
    SELECT COUNT(*) INTO assignment_count FROM task_assignments;
    
    -- Count assignments migrated from worker references
    SELECT COUNT(*) INTO migrated_worker_assignments 
    FROM task_assignments ta
    JOIN tasks t ON t.id = ta.task_id
    WHERE EXISTS (SELECT 1 FROM workers w WHERE w.id = t.assigned_to);
    
    -- Count assignments migrated from user references  
    SELECT COUNT(*) INTO migrated_user_assignments
    FROM task_assignments ta
    JOIN tasks t ON t.id = ta.task_id
    JOIN workers w ON w.id = ta.worker_id
    WHERE w.user_id = t.assigned_to;
    
    RAISE NOTICE '📊 Migration Summary:';
    RAISE NOTICE '   Total task assignments created: %', assignment_count;
    RAISE NOTICE '   Migrated from worker references: %', migrated_worker_assignments;
    RAISE NOTICE '   Migrated from user references: %', migrated_user_assignments;
END $$;
...
BEGIN
    ...
    RAISE NOTICE '   ✅ Safe migration of existing assignments';
END $$;